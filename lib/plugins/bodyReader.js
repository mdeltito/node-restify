...
function bodyReader(options) {
    ...
    function readBody(req, res, originalNext) {
        ...
        var maxBodySizeExceededMsg = 'Request body size exceeds';
        var bytesReceived = 0;
        var digest;
        var gz;
        var hash;
        ...
        function done() {
            ...
            if (maxBodySize && bytesReceived > maxBodySize) {
                msg = `${maxBodySizeExceededMsg} ${maxBodySize}`;
            ...
            if (gzError) {
                if (
                    gzError.errno === zlib.constants.Z_DATA_ERROR ||
                    gzError.errno === zlib.constants.Z_STREAM_ERROR
                ) {
        ...
        if (req.headers['content-encoding'] === undefined) {
            ...
            var inflatedBytesReceived = 0;
            gz.on('data', function onGzData(chunk) {
                // check maxBodySize for inflated request body and update
                // actual bytesReceived. this triggers the error in the `done()`
                if (maxBodySize) {
                    inflatedBytesReceived += chunk.length;

                    if (inflatedBytesReceived > maxBodySize) {
                        bytesReceived = inflatedBytesReceived;
                        maxBodySizeExceededMsg =
                            'Inflated request body size exceeds';
                        return;
                    }
                }
        ...
        req.on('data', function onRequestData(chunk) {
            // check maxBodySize for raw payload
            if (maxBodySize) {
